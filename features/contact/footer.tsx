...
const Footer = ({ totalAwards, location }: { totalAwards: number; location: string }) => {
  ...
  return (
    <div className="px-x-default absolute right-0 bottom-0 left-0 z-50 flex h-fit w-full grid-cols-3 items-center justify-between gap-5 pb-8 text-black sm:grid md:grid-cols-12">
      <SafeNumberFlow
      ...
      <Time className="hidden text-right text-sm! sm:block md:col-span-3" isDark={false} />
    </div>
  );
};
